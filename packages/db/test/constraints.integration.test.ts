...
describeWithDatabase('database guarantees', () => {
  ...
  describe('the audit trail is evidence', () => {
    it('refuses a USER action that does not name its actor', async () => {
    ...
    it('permits a SYSTEM action without a membership — a job has none', async () => {
      await inRolledBackTransaction(async (tx) => {
    ...
    it('denies DELETE on audit_events', async () => {
      await inRolledBackTransaction(async (tx) => {
        ...
        await expect(tx.auditEvent.delete({ where: { id } })).rejects.toThrow(/permission denied/i);
      });
    });

    it('denies UPDATE on security_events', async () => {
      await inRolledBackTransaction(async (tx) => {
        const id = newId();
  ...
  describe('cross-tenant references are structurally impossible', () => {
     ...
     */
    it('refuses a membership pointing at another organisation’s department', async () => {
      await inRolledBackTransaction(async (tx) => {
        await expect(
    ...
    it('permits the same reference within one organisation', async () => {
      await inRolledBackTransaction(async (tx) => {
  ...
  describe('domain invariants', () => {
    ...
    it('refuses a currency code that is not ISO-4217', async () => {
      await inRolledBackTransaction(async (tx) => {
        await expect(
          tx.organization.create({
            data: {
              id: newId(),
              slug: `bad-ccy-${newId().slice(0, 8)}`,
              name: 'Bad',
              baseCurrency: 'us ',
              countryCode: 'US',
              updatedAt: new Date(),
    ...
    it('refuses a fiscal year starting in month 13', async () => {
      await inRolledBackTransaction(async (tx) => {
        await expect(
          tx.organization.create({
            data: {
              id: newId(),
              slug: `bad-fy-${newId().slice(0, 8)}`,
              name: 'Bad',
              baseCurrency: 'USD',
              countryCode: 'US',
     ...
     */
    it('refuses an undelimited department path', async () => {
      await inRolledBackTransaction(async (tx) => {
        await expect(
    ...
    it('refuses a department that is its own parent', async () => {
      await inRolledBackTransaction(async (tx) => {
        const id = newId();
        await expect(
          tx.department.create({
     ...
     */
    it('refuses an ENTITY-scoped membership with no entities', async () => {
      await inRolledBackTransaction(async (tx) => {
        await expect(
          tx.membership.create({
            data: {
    ...
    it('refuses a session whose idle expiry outlasts its absolute expiry', async () => {
      await inRolledBackTransaction(async (tx) => {
        const now = Date.now();
        await expect(
          tx.session.create({
  ...
  describe('case-insensitive identity', () => {
    ...
    it('treats email as case-insensitive', async () => {
      await inRolledBackTransaction(async (tx) => {
        const email = `Case-${newId().slice(0, 8)}@Example.test`;
