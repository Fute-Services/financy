...
What this document *does* commit to is an **engineering posture** consistent with a future audit:
...
| NFR-MNT-003 | Architecture lint rules, enforced in CI: no cross-module imports except through a module's public index; no policy logic outside `modules/policies`; no direct BullMQ import outside the queue adapter; no money arithmetic in `apps/web`; no `PrismaClient` import outside `packages/db` and the platform database module. |
| NFR-MNT-004 | Every domain module exposes an explicit public API (`index.ts`); everything else is internal. |
| NFR-MNT-005 | Coverage floors: 90 % for `packages/core` and the policy/approval modules; 80 % overall for `apps/api`. CI fails below. |
...
| NFR-I18N-003 | Multi-currency data is supported from day one; multi-currency *consolidation* is Phase 7. |
