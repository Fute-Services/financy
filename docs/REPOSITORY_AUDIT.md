...
decision must be _made deliberately and recorded_, not defaulted into.
...
| G   | What can be reused safely?          | **Nothing in-repo.** Reusable _environment_ assets only: the running PostgreSQL 18 instance and the Node 24 toolchain.            |
...
| **Docker / Docker Desktop** | No one-command local stack. Not installed at `C:\Program Files\Docker` either.               | Use the natively installed PostgreSQL. Ship `infra/docker-compose.yml` anyway for CI and for developers who _do_ have Docker — it must not be the only supported path. ADR-0007. |
...
Covered in §4. The risk is _architectural_, not merely operational: if async work is written
