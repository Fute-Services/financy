...
change stays in the request. Everything else is enqueued *after* the commit — a job scheduled
...
interface QueuePort {
  enqueue<T extends JobName>(
    name: T, payload: JobPayload<T>, opts?: EnqueueOptions
  ): Promise<JobHandle>;

  schedule<T extends JobName>(
    name: T, payload: JobPayload<T>, runAt: Date, opts?: EnqueueOptions
  ): Promise<JobHandle>;

  registerRecurring<T extends JobName>(name: T, cron: string, payload: JobPayload<T>): Promise<void>;
...
*will* happen in production; a job that has not been proven idempotent is a job that will
