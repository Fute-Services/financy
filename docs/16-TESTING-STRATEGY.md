...
const ILLEGAL: Array<[SpendStatus, SpendStatus]> = [
  ['DRAFT', 'APPROVED'], ['REJECTED', 'APPROVED'], ['APPROVED', 'DRAFT'],
  ['CANCELLED', 'SUBMITTED'], ['FULFILLED', 'PENDING_APPROVAL'], /* …all of them */
];
...
Against a real database, because the guarantees being tested *are* database guarantees. A mocked
...
is worse tested than one at 80 % that has them, so review asks *what* is covered — the numbers only
catch regressions.
